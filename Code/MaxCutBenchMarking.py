...
def qaoa_maxcut(opt, graph, n_layers, verbose=False):
    opt.reset() # always reset optimizer at the start
    # SETUP PARAMETERS
    losses = []
...
if __name__ == "__main__":
    ...
    OPTIMIZERS = [qml.AdamOptimizer(0.05), qml.GradientDescentOptimizer(0.05),\
         qml.RotosolveOptimizer()]
    OPTIM_NAMES = ["adam", "gd", "qng","roto"]

    arr_map_over = [(i,j,str(k)) for i in OPTIM_NAMES for j in GRAPH_NAMES for k in range(1,4)]
    args_map = [(i,j,k) for i in OPTIMIZERS for j in GRAPHS for k in range(1,4)]
